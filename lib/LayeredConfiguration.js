...
class LayeredConfiguration {
    ...
    splitPath(path) {
        ...
        return path.split(this.pathSeparator).filter(item => {
            ...
            return _.isString(item) && !!item.trim();
        }).map(item => {
            return item.trim();
        });
    }
    ...
    get(path, ignoreNulls, restrictToLayer) {
        ...
        _.forEach(layerNamesToUse, layerName => {
            ...
            if (layer) {
                let res = layer.getConfigurationNode(pathArray, false);
    ...
    set(path, value, layerName) {
        ...
        let layer = this.getLayer(layerName);
        if (!layer) {
            throw new Error('Cannot write to unknown layer');
        }
        // Get the parent
        let propertyName = pathArray.pop();
        let parent = layer.getConfigurationNode(pathArray, true);
        if (value === undefined) {
